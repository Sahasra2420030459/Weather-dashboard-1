...
const WEATHER_MAP = {
    0: SVG_ICONS.sun, 1: SVG_ICONS.partlyCloudy, 2: SVG_ICONS.partlyCloudy,
    3: SVG_ICONS.cloud, 45: SVG_ICONS.fog, 48: SVG_ICONS.fog,
    51: SVG_ICONS.rain, 53: SVG_ICONS.rain, 55: SVG_ICONS.rain,
    61: SVG_ICONS.rain, 63: SVG_ICONS.rain, 65: SVG_ICONS.rain,
    71: SVG_ICONS.snow, 73: SVG_ICONS.snow, 75: SVG_ICONS.snow,
    95: SVG_ICONS.thunder, 96: SVG_ICONS.thunder
};
...
function initVoiceSearch() {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
        ...
        btn.title = 'Voice search not supported'; return;
    }
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    state.recognition = new SpeechRecognition();
    state.recognition.lang = 'en-US'; state.recognition.continuous = false; state.recognition.interimResults = false;
    ...
    state.recognition.onresult = (e) => {
        ...
        document.getElementById('cityInput').value = city; searchWeather();
    };
    state.recognition.onerror = (e) => showError('Voice search failed: ' + e.error);
}
...
function checkSevereWeather(current, daily) {
    const alerts = [];
    ...
    if (code >= 95) {
        alerts.push({
            type: 'thunderstorm',
            icon: '⛈️',
            message: 'Thunderstorm warning in effect. Stay indoors and avoid open areas.',
            severity: 'high'
        });
    }
    // Heavy Rain
    ...
    if (wind >= 40) {
        alerts.push({
            type: 'strongwind',
            icon: '💨',
            message: `Strong winds up to ${Math.round(wind)} km/h. Secure loose objects outdoors.`,
            severity: 'medium'
        });
    }
    // Heat Wave
    if (maxTemp >= 40) {
        alerts.push({
            type: 'heatwave',
            icon: '🌡️',
            message: `Extreme heat warning! High of ${Math.round(maxTemp)}°C. Stay hydrated and avoid direct sun.`,
            severity: 'high'
        });
    } else if (maxTemp >= 35) {
        alerts.push({
            type: 'heatwave',
            icon: '☀️',
            message: `Heat advisory: High of ${Math.round(maxTemp)}°C. Drink plenty of water.`,
            severity: 'medium'
        });
    }

    renderSevereAlerts(alerts);
}

function renderSevereAlerts(alerts) {
    const container = document.getElementById('severeAlerts');
...
function getChartColors() {
    ...
    return {
        ...
        temp: 'rgba(59, 130, 246, 0.8)',
        tempFill: 'rgba(59, 130, 246, 0.15)',
        humidity: 'rgba(16, 185, 129, 0.8)',
        humidityFill: 'rgba(16, 185, 129, 0.15)',
        wind: 'rgba(245, 158, 11, 0.8)',
        windFill: 'rgba(245, 158, 11, 0.15)'
    };
}
...
function createCharts(hourly) {
    ...
    for (let i = 0; i < 24; i += 3) {
        ...
        labels.push(`${hour}:00`);
        tempData.push(formatTemp(hourly.temperature_2m[idx]));
        humidityData.push(hourly.relative_humidity_2m ? hourly.relative_humidity_2m[idx] : Math.round(50 + Math.random() * 30));
        windData.push(hourly.wind_speed_10m ? hourly.wind_speed_10m[idx] : Math.round(5 + Math.random() * 20));
    }
    ...
    state.charts.temp = new Chart(tempCtx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: `Temperature (${state.isCelsius ? '°C' : '°F'})`,
                data: tempData,
    ...
    state.charts.wind = new Chart(windCtx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: `Wind Speed (${state.isCelsius ? 'km/h' : 'mph'})`,
                data: windData,
...
function updateChartsTheme() {
    ...
    createCharts(state.currentData.weather.hourly);
}
...
async function fetchWeatherData(lat, lon, cityName) {
    ...
    try {
        const weatherUrl = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,pressure_msl,wind_speed_10m,uv_index&hourly=temperature_2m,relative_humidity_2m,weather_code,precipitation_probability,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&timezone=auto&forecast_days=7`;
        ...
        } catch (e) { console.log('AQI unavailable'); }
...
function updateWeatherUI(weather, aqi, cityName) {
    ...
    // Severe Weather Alerts
    checkSevereWeather(current, daily);
    ...
    // Weather Alert (simple)
    const alertEl = document.getElementById('weatherAlert');
    if (current.weather_code >= 95) {
        alertEl.className = 'weather-alert danger'; alertEl.textContent = '⚠️ Thunderstorm warning';
    } else if (current.weather_code >= 71) {
        alertEl.className = 'weather-alert warning'; alertEl.textContent = '❄️ Snow expected';
    } else if (current.weather_code >= 61) {
        alertEl.className = 'weather-alert warning'; alertEl.textContent = '🌧️ Rain expected';
    } else { alertEl.className = ''; alertEl.textContent = ''; }
    ...
    if (aqi && aqi.current) {
        ...
        document.getElementById('aqiCircle').style.background = aqiInfo.color;
    ...
    for (let i = 0; i < daily.time.length; i++) {
        ...
        div.innerHTML = `
            ...
            <div class="daily-temps">
                ...
                <div class="daily-bar-bg"><div class="daily-bar-fill" style="width: ${Math.min((range / 20) * 100, 100)}%"></div></div>
...
async function searchWeather() {
    ...
    await searchCity(city); input.value = '';
}
...
async function searchCity(city) {
    ...
    try {
        ...
        if (!data.results || !data.results.length) { showError(`City "${city}" not found`); return; }
        const result = data.results[0];
        const name = result.name + (result.country ? `, ${result.country}` : '');
        await fetchWeatherData(result.latitude, result.longitude, name);
    } catch (error) { showError('Search failed. Please try again.'); }
}
...
function getMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        async (pos) => {
            ...
            } catch (e) { await fetchWeatherData(latitude, longitude, 'Your Location'); }
        },
...
document.addEventListener('DOMContentLoaded', () => {
    loadTheme(); loadUnit(); renderHistory(); initVoiceSearch();
